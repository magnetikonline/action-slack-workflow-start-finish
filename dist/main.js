...
var require_lib = __commonJS({
  "src/lib.js"(exports2, module2) {
    ...
    function parseArgs(core2, context) {
      ...
      data.repositoryName = core2.getInput("repository");
      if (!/^[^/ ]+\/[^/ ]+$/.test(data.repositoryName)) {
        throw new Error("input GitHub repository has unexpected format");
      }
      data.result = parseArgsResult(core2.getInput("result"));
      data.slackWebhookUrl = core2.getInput("webhook-url");
    ...
    function parseArgsRef(ref) {
      ...
      return {
        ...
        // drop `refs/heads/`, if exists
      };
    }
    function parseArgsCustomFieldList(fieldList) {
      const titleValueRegexp = /^([^|]+)\|(.+)/, resultList = [];
    ...
    function buildSlackPayload(channel, data) {
      ...
      function makeSlackLink(title, url2) {
        function escape(text) {
          text = ("" + text).replaceAll("&", "&amp;");
          text = text.replaceAll("<", "&lt;");
          return text.replaceAll(">", "&gt;");
        }
        return `<${escape(url2)}|${escape(title)}>`;
      }
      const githubServerUrl = data.githubServerUrl, githubRepoUrlBase = `${githubServerUrl}/${data.repositoryName}`, payload = {
        channel,
    ...
    function sendSlackMessage(webhookUrl, payload) {
      return new Promise(function(resolve, reject) {
        const wh = new url.URL(webhookUrl), req = https.request(
          {
